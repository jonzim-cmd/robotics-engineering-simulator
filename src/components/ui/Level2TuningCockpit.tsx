...
export const Level2TuningCockpit: React.FC = () => {
  ...
  // Lets map Ratio 3 -> 60px, Ratio 20 -> 180px
  const drivenSize = 60 + ((gearRatio - GEAR_RATIO_MIN) / (GEAR_RATIO_MAX - GEAR_RATIO_MIN)) * 120;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative">
        {/* Visualization Panel */}
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-8 relative overflow-hidden min-h-[280px] flex items-center justify-center">
          {/* Background Grid */}
          <div className="absolute inset-0 opacity-10 bg-[url('/grid.svg')] bg-center" />
          
          {/* Connection Chain (Visual Only - simple line) */}
          <div className="absolute h-2 bg-slate-800/80 border-y border-slate-600 top-1/2 -translate-y-1/2 w-[200px]" />

          {/* Gears Container */}
          <div className="flex items-center gap-12 relative z-10">
            {/* Motor Side */}
            <div className="flex flex-col items-center gap-4">
              <div className="text-xs font-mono text-cyan-400 uppercase tracking-widest">Motor</div>
              <div className="relative">
                <SpinningGear size={driverSize} color="#22d3ee" teeth={8} speed={driverSpeed} />
              </div>
              <div className="text-xs font-mono text-slate-500">{Math.round(motorSpeedFactor * 100)}% RPM</div>
            </div>

            {/* Wheel Side */}
            <div className="flex flex-col items-center gap-4 h-[200px] justify-center w-[200px]">
              <div className="text-xs font-mono text-cyan-400 uppercase tracking-widest">Antrieb</div>
              <div className="relative flex items-center justify-center transition-all duration-500 ease-out">
                <SpinningGear 
                  size={drivenSize} 
                  color="#fbbf24" 
                  teeth={Math.floor(8 * (drivenSize/60))} 
                  speed={drivenSpeed} 
                  direction={-1} 
                />
              </div>
              <div className="text-xs font-mono text-slate-500">1:{gearRatio.toFixed(1)}</div>
            </div>
          </div>
        ...
        <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 space-y-6">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full h-4 bg-slate-900 rounded-full appearance-none cursor-pointer accent-cyan-500 hover:accent-cyan-400 transition-all"
            />
          </div>
          ...
          <div>
            ...
            <input 
              ...
              disabled={isTestRunning}
              className="w-full h-4 bg-slate-900 rounded-full appearance-none cursor-pointer accent-orange-500 hover:accent-orange-400 transition-all"
            />
          </div>
        </div>
